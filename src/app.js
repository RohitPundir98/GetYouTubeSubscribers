...
// 3=> Get a particular subscriber from the database using id
  ...
  await subscriberModel
    ...
    .then((data) => {
      if (!data) {
        ...
        error = Error(`Subscriber doesn't exist with the given id: ${id}.`);
    ...
    .catch((error) => {
      ...
      res.status(400).json({
        message: `Subscriber doesn't exist with the given id: ${id}.`,
      });
    });
});
